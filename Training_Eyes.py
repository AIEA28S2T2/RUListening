...
IMG_DIMS = (224, 224, 3)  # Downscaled from 640x480 but optimal for EfficientNet
INITIAL_LR = 1e-4  # Higher learning rate with decay


# Single data directory (keeping the same path)
DATA_DIR = r'F:\Final\Dataset_Eyes\TrainingSet'


# # Single data directory (using relative path)
# DATA_DIR = os.path.join('Dataset_Eyes', 'TrainingSet')


# Updated class definitions - only two classes
    ...
    for class_idx, class_name in enumerate(CLASS_NAMES):
        ...
        for img_path in image_paths:
            try:
                ...
                # Resize from original 640x480 to model input size
...
# Enhanced callbacks - updated model name to "Eyes"
callbacks = [
    EarlyStopping(monitor='val_accuracy', patience=15, restore_best_weights=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, min_lr=1e-7),
    ModelCheckpoint('Eyes.keras', save_best_only=True, 
...
def plot_training(history):
    ...
    plt.tight_layout()
    plt.savefig("training_curves_eyes.png", dpi=300)
    plt.show()
...
plt.tight_layout()
plt.savefig("confusion_matrix_eyes.png", dpi=300)
plt.show()
...
# Save final model with updated name
model.save("Eyes.keras")
print("Model saved as 'Eyes.keras'")